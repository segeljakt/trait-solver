...
pub struct Sources<'a>(Vec<(String, Source<&'a str>)>);
...
impl<'a> Sources<'a> {
    ...
    pub fn add(&mut self, name: impl ToString, data: &'a str) -> u16 {
        let id = self.0.len() as u16;
        self.0.push((name.to_string(), Source::from(data)));
        id
    }
}

impl<'a> std::ops::Index<u16> for Sources<'a> {
    type Output = Source<&'a str>;
...
impl<'a> std::fmt::Debug for Sources<'a> {
...
impl<'a> ariadne::Cache<u16> for Sources<'a> {
    type Storage = &'a str;
